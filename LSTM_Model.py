...
class LSTMModel:
    def __init__(self, csv_file_path, train_test_split_ratio=0.8, num_time_steps=10, num_features=5, num_hidden_units=50):
    ...
    def preprocess(self):
        ...
        self.scaled_data = self.scaler.fit_transform(self.df[['open', 'high', 'low', 'close', 'volume']])

        # Split the data into training and testing sets
        num_training_samples = int(len(self.scaled_data) * self.train_test_split_ratio)
        self.x_train = []
        self.y_train = []
        for i in range(self.num_time_steps, num_training_samples):
            self.x_train.append(self.scaled_data[i - self.num_time_steps:i, :])
            self.y_train.append(self.scaled_data[i, 3])  # use the close price as the label
        ...
        for i in range(num_training_samples, len(self.scaled_data)):
            ...
            self.y_test.append(self.scaled_data[i, 3])  # use the close price as the label
